<x-app-layout>
    <x-slot name="header">
        <div
            ...
            <div>
                <x-breadcrumb :items="[['label' => 'Sampah', 'url' => route('admin.waste.index')], ['label' => 'Edit']]" />

                <h2 class="font-semibold text-xl text-gray-900 leading-tight">
                    {{ __('Edit Item Sampah') }}
                </h2>
                <span class="text-sm text-gray-400">
                    Perbarui data item sampah
                </span>
            </div>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg px-6 py-6">
            ...
            <!-- Form -->
            <form action="{{ route('admin.waste.update', $waste->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="category" id="category"
                                ...
                                <option value="organik"
                                    {{ old('category', $waste->category) == 'organik' ? 'selected' : '' }}>Organik
                                </option>
                                <option value="anorganik"
                                    {{ old('category', $waste->category) == 'anorganik' ? 'selected' : '' }}>Anorganik
                                </option>
                                <option value="b3"
                                    {{ old('category', $waste->category) == 'b3' ? 'selected' : '' }}>B3 (Bahan
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="3"
                                ...
                                placeholder="Deskripsi singkat tentang item sampah">{{ old('description', $waste->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="composition" id="composition" rows="2"
                                ...
                                placeholder="contoh: PET (Polyethylene Terephthalate)">{{ old('composition', $waste->composition) }}</textarea>
                            @error('composition')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="impact" id="impact" rows="3"
                                ...
                                placeholder="Jelaskan dampak lingkungan jika tidak dikelola dengan baik">{{ old('impact', $waste->impact) }}</textarea>
                            @error('impact')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="handling" id="handling" rows="3"
                                ...
                                placeholder="Cara menangani dan membuang sampah ini dengan benar">{{ old('handling', $waste->handling) }}</textarea>
                            @error('handling')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="recycling" id="recycling" rows="2"
                                ...
                                placeholder="Sampah ini dapat didaur ulang menjadi apa?">{{ old('recycling', $waste->recycling) }}</textarea>
                            @error('recycling')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mt-2 flex flex-col space-y-3">
                                ...
                                <p class="text-xs text-gray-500">
                                    ...
                                    @if ($waste->image_path)
                                        <br>
                                        <span class="text-yellow-600">Upload gambar baru akan mengganti yang
                                            lama</span>
                                    @endif
                                </p>
                                ...
                                <label for="image"
                                    ...
                                    {{ $waste->image_path ? 'Ganti Gambar' : 'Pilih Gambar' }}
                                </label>
